...
async function obtenerAsesoriaVideoAutorizada(idAsesoria, usuario) {
  ...
  if (asesoria.tipo === "individual" && !["pagado", "aprobado"].includes(String(asesoria.estado_pago || "").toLowerCase())) {
    return {
      ok: false,
      status: 402,
      message: "La videollamada se libera cuando el alumno paga la asesoría"
    };
  }
...
exports.crearAsesoria = async (req, res) => {
  try {
    ...
    const asesor = await getQuery(
      `SELECT u.id_usuario, u.rol, COALESCE(a.precio_individual, ?) AS precio_individual
       FROM usuarios u
       LEFT JOIN asesores a ON a.id_usuario = u.id_usuario
       WHERE u.id_usuario = ?`,
      [Number(process.env.PRECIO_BASE_ASESORIA || process.env.PRECIO_MINIMO_ASESORIA || 100), id_asesor]
    );
    ...
    const result = await runQuery(
      `INSERT INTO asesorias (id_alumno, id_asesor, mensaje, tipo, cupo_maximo, precio, estado_pago)
       VALUES (?, ?, ?, 'individual', 1, ?, 'sin_pago')`,
      [id_alumno, id_asesor, mensaje || null, Number(asesor.precio_individual || process.env.PRECIO_BASE_ASESORIA || process.env.PRECIO_MINIMO_ASESORIA || 100)]
    );
...
exports.crearAsesoriaGrupal = async (req, res) => {
  try {
    ...
    const { fecha, hora, mensaje, cupo_maximo, precio } = req.body;
    ...
    const precioFinal = Number(precio || process.env.PRECIO_BASE_ASESORIA || process.env.PRECIO_MINIMO_ASESORIA || 100);

    if (!precioFinal || precioFinal < 1) {
      return res.status(400).json({
        ok: false,
        message: "El precio de la asesoría debe ser válido"
      });
    }
    ...
    const result = await runQuery(
      `INSERT INTO asesorias (
        ...
        precio,
        estado_pago
      ) VALUES (?, ?, 'aceptada', ?, ?, 'grupal', ?, ?, ?, 'sin_pago')`,
      [null, id_asesor, fecha, hora, mensaje || null, Number(cupo_maximo), precioFinal]
    );
...
exports.reagendarAsesoria = async (req, res) => {
  try {
    const { id } = req.params;
    const id_asesor = req.user.id_usuario;
    ...
    if (!fecha || !hora) {
      return res.status(400).json({
        ok: false,
        message: "Debes indicar la nueva fecha y hora"
      });
    }

    const asesoria = await getQuery(
      `SELECT id_asesoria, id_asesor, estado, fecha, hora, tipo
       FROM asesorias
       WHERE id_asesoria = ?`,
      [id]
    );
    ...
    if (Number(asesoria.id_asesor) !== Number(id_asesor)) {
      return res.status(403).json({
        ok: false,
        message: "No puedes reagendar una asesoría que no te pertenece"
      });
    }
    ...
    if (asesoria.estado !== "aceptada") {
      return res.status(400).json({
        ok: false,
        message: "Solo puedes reagendar asesorías aceptadas"
      });
    }

    if (String(asesoria.fecha || "") === String(fecha) && String(asesoria.hora || "") === String(hora)) {
      return res.status(400).json({
        ok: false,
        message: "La nueva fecha y hora deben ser diferentes a la agenda actual"
      });
    }
    ...
    await runQuery(
      `UPDATE asesorias
       SET
         ...
         reagendada = TRUE,
         motivo_reagenda = ?,
         fecha_reagenda = CURRENT_TIMESTAMP,
         reagendada_por = ?
       WHERE id_asesoria = ?`,
      [fecha, hora, motivo || null, id_asesor, id]
    );

    return res.json({
      ok: true,
      message: "Asesoría reagendada correctamente"
    });
  } catch (error) {
    console.error("Error al reagendar asesoría:", error);
    return res.status(500).json({
      ok: false,
      message: "Error al reagendar asesoría"
    });
  }
};
