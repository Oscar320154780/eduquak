...
// ASESOR: eliminar su cuestionario
...
// ALUMNO: ver preguntas de un cuestionario aprobado
exports.obtenerPreguntasCuestionario = async (req, res) => {
  try {
    const { id } = req.params;

    const cuestionario = await getQuery(
      `SELECT id_cuestionario, estado_revision, titulo
       FROM cuestionarios
       WHERE id_cuestionario = ?`,
      [id]
    );
...
// ALUMNO: responder cuestionario y calcular puntaje
...
// ADMIN: ver preguntas de cualquier cuestionario para revisión
